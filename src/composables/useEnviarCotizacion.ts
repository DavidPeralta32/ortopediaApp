...
export async function useEnviarCotizacion() {
    ...
    /** Local 
    const SERVICE_ID = "service_lkr6hz4";
    const TEMPLATE_ID = "template_h375lsl";
    const PUBLIC_KEY = "hRfNYeyPVcY8WsDFN";
    const MI_CORREO_NEGOCIO = 'davi32peralta@gmail.com';*/

    /**
     * Genera una URL de WhatsApp estructurada con los datos de la cotización
     */
    const generarEnlaceWhatsApp = (datos: EnviarCotizacionInput): string => {
        ...
        if (datos.productos && datos.productos.length > 0) {
            ...
            datos.productos.forEach(item => {
                // 🌟 NUEVO: Captura e imprime la talla en el mensaje de WhatsApp
                const talla = item.tallaSeleccionada || 'Universal';
                texto += `- ${item.name} [Talla: ${talla}] (Cant: ${item.quantity})\n`;
            });
        }
    ...
    const generarTablaProductosHTML = (productos?: Producto[]): string => {
        ...
        productos.forEach(item => {
            // 🌟 NUEVO: Captura e imprime la talla en la celda del correo HTML
            const talla = item.tallaSeleccionada || 'Universal';
            ...
            filas += `
                <tr style="border-bottom: 1px solid #eeeeee;">
                    ...
                    <td style="padding: 12px 8px; font-size: 14px; color: #333333; vertical-align: middle;">
                        ...
                        <div style="font-size: 12px; color: #666666; margin-top: 4px;">Talla: <span style="font-weight: bold; color: #1C90A1;">${talla}</span></div>
                    </td>
    ...
    const ejecutarFallbackManual = (datos: EnviarCotizacionInput) => {
        ...
        if (datos.productos && datos.productos.length > 0) {
            // 🌟 NUEVO: Mapeo de talla en el string del mailto clásico
            cuerpo += `\nProductos solicitados:\n` + datos.productos.map(p => {
                const t = p.tallaSeleccionada || 'Universal';
                return `- ${p.name} (Talla: ${t}) (Cant: ${p.quantity})`;
            }).join('\n');
        }
